...
export function generateRoomSpecAdminEmail(roomData: any, userEmail: string, isFromGenerator: boolean = false): string {
  ...
  // Generate comprehensive specification details
  const fullSpec = generateFullRoomSpecification(roomData);
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
      <div style="background: white; border-radius: 8px; padding: 25px; border-left: 4px solid #10b981;">
        ...
        <p style="color: #64748b; margin: 0 0 25px 0;"><strong>Requested:</strong> ${timestamp}</p>
        ...
        <div style="background: #f8fafc; padding: 15px; border-radius: 6px; margin-bottom: 20px;">
          ...
          <p style="color: #475569; margin: 5px 0 0 0;"><strong>Session ID:</strong> ${roomData.sessionId || 'N/A'}</p>
        </div>
        ...
        <div style="background: #f0fdf4; padding: 15px; border-radius: 6px; margin-bottom: 20px;">
          ...
          <p style="color: #047857; margin: 0 0 10px 0;"><strong>Description:</strong> ${roomData.description || roomData.purpose}</p>
          ${roomData.category ? `<p style="color: #047857; margin: 5px 0 0 0;"><strong>Category:</strong> ${roomData.category}</p>` : ''}
          ${roomData.engagement ? `<p style="color: #047857; margin: 5px 0 0 0;"><strong>Engagement Score:</strong> ${roomData.engagement}%</p>` : ''}
          ${roomData.tags ? `<p style="color: #047857; margin: 5px 0 0 0;"><strong>Tags:</strong> ${roomData.tags.join(', ')}</p>` : ''}
        </div>
...
${fullSpec}
        ...
        ${roomData.technicalRequirements ? `
        ...
        <div style="background: #fef3c7; padding: 15px; border-radius: 6px; margin-bottom: 20px;">
          ...
          <p style="color: #92400e; margin: 0; font-size: 14px;">
            <strong>Time Spent:</strong> ${roomData.analytics.timeSpent || 'N/A'}<br>
            <strong>Interactions:</strong> ${roomData.analytics.interactions || 'N/A'}<br>
            <strong>Interest Level:</strong> ${roomData.analytics.interestLevel || 'N/A'}
          </p>
        </div>
        ` : ''}
...
function generateFullRoomSpecification(roomData: any): string {
  return JSON.stringify({
    roomId: roomData.id,
    title: roomData.title || roomData.name,
    description: roomData.description || roomData.purpose,
    category: roomData.category,
    engagement: roomData.engagement,
    tags: roomData.tags,
    targetAudience: roomData.targetAudience,
    completeness: roomData.completeness,
    features: roomData.features,
    wireframes: roomData.wireframes,
    technicalSpecs: roomData.technicalSpecs,
    implementation: roomData.implementation,
    timeline: roomData.timeline,
    budget: roomData.budget,
    analytics: roomData.analytics,
    userJourney: roomData.userJourney,
    timestamp: new Date().toISOString(),
    sessionData: roomData.sessionData
  }, null, 2);
}
